...
{
    if (code == LOCK)
        mutex_error(pthread_mutex_lock(mutex), code);         //MUTEX LOCK
    else if (code == UNLOCK)
        mutex_error(pthread_mutex_unlock(mutex), code);       //MUTEX UNLOCK
    else if (code == INIT)
        mutex_error(pthread_mutex_init(mutex, NULL), code);   //MUTEX INIT
    else if (code == DESTROY)
        mutex_error(pthread_mutex_destroy(mutex), code);      //MUTEX DESTROY
    else
